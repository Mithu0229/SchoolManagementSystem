...
@Component({
    ...
    template: `
        <ng-container>
            ...
            <a
                *ngIf="item.routerLink && !item.items && item.visible !== false && item.isSidebarmenu !== false"
...
export class AppMenuitem {
    ...
    ngOnInit() {
        ...
        //     this.updateActiveStateFromRoute();
        // }
        this.activateMenuByRoute(this.item, this.key);
    }

    activateMenuByRoute(item: MenuItem, key: string): boolean {
        if (
            item.routerLink &&
            ...
            this.layoutService.onMenuStateChange({ key: key, routeEvent: true });
            this.active = true;
            return true;
        }
        ...
        if (item.items) {
            for (let i = 0; i < item.items.length; i++) {
                ...
                const activated = this.activateMenuByRoute(item.items[i], childKey);
                if (activated) {
                    this.active = true;
                    return true;
                }
