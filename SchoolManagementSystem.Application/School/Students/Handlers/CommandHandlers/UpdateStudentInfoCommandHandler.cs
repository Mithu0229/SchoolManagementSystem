...
public class UpdateStudentInfoCommandHandler : IHttpRequestHandler<UpdateStudentInfoCommand>
{
    private IUnitOfWork _unitOfWork;
    private readonly ICurrentUserService _currentUserService;

    public UpdateStudentInfoCommandHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUserService)
    {
        _unitOfWork = unitOfWork;
        _currentUserService = currentUserService;
    }

    public async Task<IResult> Handle(UpdateStudentInfoCommand request, CancellationToken cancellationToken)
    {
        try
        {
            if (request is null)
            ...
            var studentInfo = await _unitOfWork.StudentInfoRepository.GetAllNoneDeleted()
                .FirstOrDefaultAsync(s => s.Id == request.StudentInfo.Id);
